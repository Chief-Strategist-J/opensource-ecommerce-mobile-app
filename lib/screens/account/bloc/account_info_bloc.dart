...
class AccountInfoBloc extends Bloc<AccountInfoBaseEvent, AccountInfoBaseState> {
  ...
  void mapEventToState(AccountInfoBaseEvent event, Emitter<AccountInfoBaseState> emit) async {
    if (event is AccountInfoDetailsEvent) {
      ...
      try {
        AccountUpdate accountUpdate = await repository!.callAccountUpdateApi(event.firstName ?? "", event.lastName ?? "", event.email ?? "", event.gender ?? "", event.dob ?? "", event.phone ?? "", event.oldPassword ?? "", event.password ?? "", event.confirmPassword ?? "", event.avatar!);
